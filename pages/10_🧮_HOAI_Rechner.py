...
with c1:
    firma = st.text_input("Firma", value=loaded_form.get("firma", loaded_meta.get("Firma", "")))
    email = st.text_input("E-Mail", value=loaded_form.get("email", loaded_meta.get("E-Mail", "")))
    telefon = st.text_input("Telefon", value=loaded_form.get("telefon", loaded_meta.get("Telefon", "")))

with c2:
    adresse = st.text_area("Adresse", value=loaded_form.get("adresse", loaded_meta.get("Adresse", "")))
...
with c1:
    projektname = st.text_input("Projekt", value=loaded_form.get("projektname", loaded_meta.get("Projekt", "")))
    bauherr = st.text_input("Bauherr", value=loaded_form.get("bauherr", loaded_meta.get("Bauherr", "")))

with c2:
    ort = st.text_input("Ort", value=loaded_form.get("ort", loaded_meta.get("Ort", "")))
    datum = st.date_input("Datum", value=parse_date(loaded_form.get("datum", loaded_meta.get("Datum", ""))))
...
with c4:
    mwst = st.number_input("MwSt %", value=float(loaded_form.get("mwst", 19.0)), step=1.0)
...
with c1:
    umbau = st.number_input("Umbauzuschlag %", value=float(loaded_form.get("umbau", 0.0)))
with c2:
    umbau_order = st.number_input("Reihenfolge Umbau", value=int(loaded_form.get("umbau_order", 1)), step=1)
with c3:
...
with c3:
    nebenkosten = st.number_input("Nebenkosten %", value=float(loaded_form.get("nebenkosten", 0.0)))
with c4:
...
if st.button("🔵 Angebot berechnen"):
    try:
        ...
        adjustments = [
            {"name": "Umbauzuschlag", "value": umbau, "order": umbau_order, "kind": "percent_plus_current"},
            {
            ...
            {"name": "Nebenkosten", "value": nebenkosten, "order": nebenkosten_order, "kind": "percent_plus_current"},
            {
...
if st.session_state.result:
    ...
    excel_file = export_excel(result["meta"], result["params"], result["lph_rows"], result["steps"])
    word_file = export_word(result["meta"], result["params"], result["lph_rows"], result["steps"])
    pdf_file = export_pdf(result["meta"], result["params"], result["lph_rows"], result["steps"])
