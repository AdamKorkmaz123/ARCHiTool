...
def top_navigation():
    html([
        ...
        '<div><div class="arch-logo-title">ARCHiTool</div>',
        '<div class="arch-logo-subtitle">Architecture Intelligence Platform</div></div>',
        '</div></a>',

        '<div class="arch-menu">',

        '<div class="arch-menu-item">Plattform',
        ...
        '<div class="arch-menu-item">Projektplanung',
        '<div class="arch-dropdown mega-medium">',
        ...
        '<div class="arch-menu-item">Ausschreibung / LV',
        '<div class="arch-dropdown mega-medium">',
        ...
        '<div class="arch-menu-item">Bauleitung',
        '<div class="arch-dropdown mega-medium">',
        ...
        '<div class="arch-menu-item">Kosten',
        '<div class="arch-dropdown mega-medium">',
        ...
        '</div>',
        '<div class="arch-top-actions"><a href="/Login" class="arch-login-btn">Login</a></div>',
        '</div>',
    ])


def hero_section(badge, title, text, primary_label, primary_link, secondary_label, secondary_link):
    html([
        ...
        f'<a href="{primary_link}" class="hero-primary">{primary_label}</a>',
        f'<a href="{secondary_link}" class="hero-secondary">{secondary_label}</a>',
        '</div></div>',
    ])
...
def module_card(status, icon, title, text, link="#"):
    ...
    html([
        f'<a href="{link}" class="module-card-link">',
...
def feature_strip(title, text, features):
    items = ''.join([f'<div class="feature-item">{x}</div>' for x in features])
    html([
...
def info_panel(title, text, icon="ℹ️"):
    html([
        ...
        f'<div class="info-panel-icon">{icon}</div>',
        '<div>',
        f'<h3>{title}</h3>',
        f'<p>{text}</p>',
        '</div></div>',
...
def premium_banner(title, text, button_label=None, button_link="#"):
    button = f'<a href="{button_link}" class="banner-button">{button_label}</a>' if button_label else ''
    html([
...
def footer():
    html([
        ...
        '<div>',
        '<h2>ARCHiTool</h2>',
        '<p>Digitale Arbeitsplattform für Architektur- und Ingenieurbüros. '
        'HOAI, Projektplanung, Ausschreibung, Bauleitung und Kostenmanagement '
        'in einem modularen professionellen System.</p>',
        '</div>',
